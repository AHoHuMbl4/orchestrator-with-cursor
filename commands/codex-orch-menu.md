...
argument-hint: "--set key=value ... | --show | --task текст --session <id> | --reset-template"
---
...
- задача сессии: `--task "текст" --session <id>` (или env ORCH_SESSION_ID)
- показать compass сессии: `--show --session <id>`
- глобальный шаблон (редко): `--task "текст" --global-template`
- ремонт загрязнённого шаблона: `--reset-template`
