...
- **Граница:** для крупных баз режим без репо — обходной путь; штатный
  путь Cursor — с подключённым репозиторием (агент клонирует и исследует
  базу сам). Включается владельцем; без него протокол выше — стандарт.
