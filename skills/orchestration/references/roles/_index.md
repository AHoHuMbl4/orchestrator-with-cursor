...
| Смотритель простоты | code/simplicity-warden.md | гейт простоты на приёмке диффа (не техдолг-аудитор: тот меряет пост-фактум) |
...
| Роль | Файл | Когда |
|---|---|---|
| Генерал фронта | meta/front-general.md | узкий фронт большого проекта; умная модель; декомпозиция + волны cursor + выжимка вверх |
| Полковник подзадачи | meta/front-colonel.md | командир ОДНОЙ подзадачи фронта; cursor; свита: советники/критики/читатели сырья |
| Наблюдатель фронтов | meta/front-observer.md | cursor-прогон командующего (run-exec) на КТ между волнами; власть 0; курс/швы/правда докладов/цель владельца |
| Военный прокурор | meta/front-prosecutor.md | cursor-прогон командующего (run-exec) на каждую волну фронта; власть 0; перекрёстный доступ / прямая координация / дубли; только командующему |
| Советник командира (командующий/генерал/полковник) | meta/opportunity-advisor.md | советник командира (локальный прогон): развилка → запускает облачных разведчиков web-scout, собирает варианты с источниками, рекомендует |
...
| Инспектор вызовов | meta/invocation-inspector.md | контрольные точки: дерево вызовов по journal.jsonl; правила иерархии; отчёт прокурору и владельцу; власть 0 |

---

**Всего: 155 ролей** (каскадный выбор: домен → поддомен → роль) (+ фабрика ролей для нестандартных)
