...
Агент (автоматически):
  ...
  5. Каскадный выбор роли: домен→поддомен→роль (149 узких специалистов)
...
Большие проекты — цепочка: командующий → наблюдатели и прокурор
(cursor-прогоны через `run-exec`: наблюдатель на контрольных точках между
волнами, прокурор на каждую волну фронта) + генералы фронтов (умные субагенты
движка) → полковники подзадач (cursor-агенты, роль `front-colonel`) →
исполнители (cursor). Полковник ведёт одну подзадачу (3–5 работ, мини-compass
≤ лимита, один уровень вложенности; эскалация после 2 неудачных кругов) и
обязан работать со свитой: советники на развилках (`opportunity-advisor`,
cloud), критики плана (`fact-checker` / `code-reviewer`), критики приёмки
каждой работы, читатели сырья (`raw-brief-synthesizer` — выжимки вместо
чтения сырья целиком), `git-warden` на код-волнах, `docs-keeper` на значимых
волнах. Роли — только из каталога `roles/_index.md`; нет → фабрика по шаблону
с атрибуцией. Граф фронтов — `.orchestration/fronts.json`, продвижение
волнами по deps. План каждого уровня идёт через критиков. Панель показывает
дерево фронтов (волны, статусы, JSON-редактор).

## Итоги прогонов и надёжность

Отчёты исполнителей/критиков заканчиваются строкой
`Вердикт: OK` / `Вердикт: PROBLEMS: <список>` / `Вердикт: BLOCKED: <причина>`
и доказательствами. Облачные прогоны
