...
## Кастомные и фабричные роли при обновлении

Роли, созданные фабрикой или вручную после установки, и их строки в
`skills/orchestration/references/roles/_index.md` — легальны: каталог ролей
живой. Обновление kit из-за них не останавливается: проверка целостности
делает исключение только для `_index.md`. Кастомные файлы ролей в
`SHA256SUMS` не входят и не проверяются — так и задумано. Жёлтое
предупреждение установщика о локально расширенном каталоге — норма, не ошибка.
...
`Вердикт: OK` / `Вердикт: PROBLEMS: <список>` / `Вердикт: BLOCKED: <причина>` + доказательства. Код-прогоны — через
